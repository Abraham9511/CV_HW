 ...
 * atan2(a, b) = fast_atan(a, b) given max(abs(a), abs(n)) > EPS
 * 40% time
 */
...
GaussianPyramid::GaussianPyramid(const Mat32f& m, int num_scale):
    nscale(num_scale),
    data(num_scale), msg(num_scale), ort(num_scale),
        ...
        REPL(x,1,w-1) {
            if (between(y,1,h-1)) {
                float dy = orig_plus[x] - orig_minus[x],
                      dx = ort_row[x+1] - ort_row[x-1];
                mag_row[i] = hypotf(dx,dy);
    ...
    origw(mat.width()), origh(mat.height())
{
    REP(i, noctave) {
        if (!i)
            pyramid.emplace_back(mat, nscale);
        else {
            ...
            resize(mat, resized);
            pyramid.emplace_back(resized, nscale);
        }
    }
}
